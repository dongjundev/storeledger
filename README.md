...
마진    = 판매가 + 구매자 배송비 − 원가 − 판매자 배송비 − 기타비용 − 수수료
마진율  = 마진 ÷ 판매가 × 100     (소수 첫째 자리)
```

- 구매자 배송비는 구매자가 결제하는 배송비(판매자 수입), 판매자 배송비는 택배사에 실제로 내는 비용입니다.
...
- 이전 버전의 "개당 배송비 · 판매자 부담"은 이제 **판매자 배송비**, 즉 택배사에 실제로 내는 비용으로 계산됩니다.
  구매자가 배송비를 내는 상품을 예전에 배송비 0으로 적어 두었다면 판매자 배송비와 구매자 배송비를 다시 입력하세요.
- 대시보드의 매출은 상품 판매액(수량 × 판매 단가)이고, 구매자 배송비는 매출에 넣지 않고 이익에만 반영합니다.
- 판매 기록의 이익은 실제 판매 단가(할인 반영)와 **그 기록에 적어 둔 개당 원가**로 계산합니다. 달러로 매입해 환율에 따라
  매입가가 달라지면, 판매를 기록할 때 그 건의 실제 원가를 적으세요. 상품에 적어 둔 원가가 기본으로 들어가니 평소에는 그냥 두면 됩니다.
  한 번 저장하면 나중에 상품 원가를 바꿔도 지난 기록의 이익은 흔들리지 않습니다.
- 이 기능이 생기기 전에 저장한 판매 기록에는 원가가 없어서, 그런 기록은 지금도 상품의 현재 원가를 따라갑니다.
  얼려 두고 싶으면 그 기록을 한 번 수정해서 저장하세요.
- 배송비·기타 비용·수수료율은 상품 값을 그대로 씁니다. 건별로 다른 것은 판매 단가와 원가뿐입니다.
- 판매자 배송비·구매자 배송비·기타 비용은 개당 금액입니다. 여러 개를 한 상자로 보내는 주문도 개당으로 계산합니다.
...
| GET / POST | `/api/sales?from=&to=` | 판매 기록 목록(기본 최근 30일) / 등록 (`unitPrice` 생략 시 상품 판매가, `unitCost` 생략 시 상품 원가) |
